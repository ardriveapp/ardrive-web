  ...
  group('AssignNameBloc', () {
    ...
    group('LoadNames', () {
      test('emits [LoadingNames, NamesLoaded] when LoadNames is added',
        ...
        when(() => mockArnsRepository.getARNSNameModelsForWallet(walletAddress))
            .thenAnswer((_) async => nameModels);
        ...
        await expectLater(
          ...
          emitsInOrder([
            ...
            isA<NamesLoaded>().having(
                (state) => state.nameModels, 'nameModels', equals(nameModels)),
          ]),
        );
      });
    ...
    group('SelectName', () {
      test(
        ...
        final selectedName = nameModels[1];
        final undernames = [
          const ARNSUndername(
              name: 'undername1',
              domain: 'domain1.ar',
        ...
        await expectLater(
          ...
          emits(
            isA<NamesLoaded>()
                .having((state) => state.nameModels, 'nameModels', nameModels)
      ...
      group('LoadUndernames', () {
        test(
          ...
          final selectedName = nameModels[0];
          final undernames = [
            const ARNSUndername(
              name: 'undername1',
              domain: 'domain1.ar',
          ...
          when(() => mockArnsRepository.getARNSUndernames(ANTRecord(
                  domain: selectedName.name,
                  processId: selectedName.processId)))
              .thenAnswer((_) async => undernames);

          assignNameBloc.add(SelectName(selectedName));
          assignNameBloc.emit(NamesLoaded(
            nameModels: nameModels,
          ...
          await expectLater(
            ...
            emitsInOrder([
              ...
              isA<UndernamesLoaded>()
                  ...
                  .having((state) => state.selectedUndername,
                      'selectedUndername', null),
            ]),
          );

          verify(() => mockArnsRepository.getARNSUndernames(ANTRecord(
              domain: selectedName.name,
              processId: selectedName.processId))).called(1);
        });
      });
      ...
      //         when(() => mockFileDataTableItem.driveId)
      //             .thenReturn('test_drive_id');
      //         final nameModels = [
      //           const ArNSNameModel(
      //             name: 'test1.ar',
      //             processId: 'process1',
      ...
      //         when(() => mockAuth.getWalletAddress())
      //             .thenAnswer((_) async => walletAddress);
      //         when(() => mockArnsRepository.getARNSNameModelsForWallet(
      //             walletAddress)).thenAnswer((_) async => nameModels);
      //         when(() => mockArnsRepository.getAntRecordsForWallet(walletAddress))
      //             .thenAnswer((_) async => antRecords);
      ...
      //         bloc.add(const SelectName(ArNSNameModel(
      //           name: 'domain',
      ...
      //           .thenAnswer((_) async => walletAddress);
      //       when(() =>
      //               mockArnsRepository.getARNSNameModelsForWallet(walletAddress))
      //           .thenAnswer((_) async => nameModels);
