...
class DriveDao extends DatabaseAccessor<Database> with _$DriveDaoMixin {
  ...
  Future<void> generateFsEntryPaths(
    String driveId,
    Map<String, FolderEntity> foldersByIdMap,
    Map<String, FileEntity> filesByIdMap,
  ) async {
    ...
    for (final folder in foldersByIdMap.values) {
      ...
      final tree = await getFolderTree(driveId, folder.id!);
    ...
    Future<void> updateFolderTree(FolderNode node, String parentPath) async {
      ...
      await updateFolderById(driveId, folderId)
      ...
      for (final staleFileId in node.files.keys) {
        ...
        await updateFileById(driveId, staleFileId)
    ...
    for (final staleOrphanFile in staleOrphanFiles) {
      if (staleOrphanFile.parentFolderId!.isNotEmpty) {
        final parentPath = await folderById(
                driveId: driveId, folderId: staleOrphanFile.parentFolderId!)
            .map((f) => f.path)
            .getSingleOrNull();

        if (parentPath != null) {
          final filePath = parentPath + '/' + staleOrphanFile.name!;

          await writeToFile(
            FileEntriesCompanion(
              id: Value(staleOrphanFile.id!),
              driveId: Value(staleOrphanFile.driveId!),
              path: Value(filePath),
            ),
          );
        }
