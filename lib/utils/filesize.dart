...
String filesizeInUnit(
  double sizeToParseInBytes,
  FileSizeUnit outputFileSizeUnit, [
  int round = 2,
]) {
  double finalSize;

  switch (outputFileSizeUnit) {
    /// Convert to bytes
    case FileSizeUnit.bytes:
      finalSize = sizeToParseInBytes;
      break;

    /// Convert to kilobytes
    case FileSizeUnit.kilobytes:
      finalSize = sizeToParseInBytes / pow(2, 10);
      break;

    /// Convert to megabytes
    case FileSizeUnit.megabytes:
      finalSize = sizeToParseInBytes / pow(2, 20);
      break;

    /// Convert to gigabytes
    case FileSizeUnit.gigabytes:
      finalSize = sizeToParseInBytes / pow(2, 30);
      break;
  }

  return finalSize.toStringAsFixed(round);
}
