// Based on code for `StreamSplitter` from `package:async`
// The only difference is that it pauses the stream when any branch is paused,
// rather than when all branches are paused. This prevents data buildup for 
// slow listeners.
...
/// been called.
class StreamSplitterLockStep<T> {
  /// The wrapped stream.
  ...
  /// then closing the [StreamSplitterLockStep].
  static List<Stream<T>> splitFrom<T>(Stream<T> stream, [int? count]) {
    count ??= 2;
    var splitter = StreamSplitterLockStep<T>(stream);
    var streams = List<Stream<T>>.generate(count, (_) => splitter.split());
    splitter.close();
    return streams;
  }

  StreamSplitterLockStep(this._stream);
