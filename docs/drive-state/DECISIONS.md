...
| D2 | Revision depth | **Current state only** for v1 — entries, not full revision history. | Halves the payload and covers everything the explorer renders. History stays available: the range beyond the artifact is snapshots and GraphQL exactly as now, and the activity view already reads revisions from those. Recorded as the lever that keeps the full-copy policy (§3.4) affordable. |
...
multi-part artifacts (D5), public drives (proposal §2.6), revision history (D2).
