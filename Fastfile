...
lane :build_autogenerated_code do |options|
  isProd = options[:type] == 'production'
  isStaging = options[:type] == 'staging'
  sh_on_root(command: "flutter gen-l10n")
  sh_on_root(command: if isProd || isStaging then "flutter pub run build_runner build --dart-define=environment=production --dart-define=SENTRY_DSN=#{ENV["SENTRY_DSN"]} --dart-define=SENTRY_PROJECT=#{ENV["SENTRY_PROJECT"]} --dart-define=SENTRY_AUTH_TOKEN=#{ENV["SENTRY_AUTH_TOKEN"]} --dart-define=SENTRY_ORG=#{ENV["SENTRY_ORG"]} --delete-conflicting-outputs" else "flutter pub run build_runner build --delete-conflicting-outputs" end)
end
